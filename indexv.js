...
document.getElementById("get_for_userxxc").addEventListener("click", async () => {
  const { isConfirmed, value } = await Swal.fire({
    ...
    html: `
      ...
      <input type="text" id="phone-input" class="swal2-input" placeholder="Ex: 0123456" style="margin-bottom:10px; width:60%; max-width:300px;"/>
  <label id="swal-label" for="reason-input">La raison du rétrait</label>
      ...
      <input
        ...
        if(partsComma.length > 2) {
            val = partsComma[0] + ',' + partsComma.slice(1).join('');
        }

        const partsDot = val.split('.');
        if(partsDot.length > 2) {
            val = partsDot[0] + '.' + partsDot.slice(1).join('');
        }
        ...
        const previewBox = document.getElementById('withdraw-preview');
        const previewAmount = document.getElementById('preview-amount');
        const previewFees = document.getElementById('preview-fees');
        ...
        if(!isNaN(amount) && amount > 0){
            ...
            previewFees.textContent = fees.toFixed(2) + ' $';
...
">
    ...
    <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
        <span>Frais (5%) :</span>
        <strong id="preview-fees">0 $</strong>
    </div>
    ...
    <div style="display:flex;justify-content:space-between;font-size:15px;">
        ...
        <strong id="preview-total" style="color:#2563eb;">0 $</strong>
    </div>
</div>
        `,
    focusConfirm: false,
  ...
  try {
    ...
    if (newBalanceCents >= 0) {
      ...
      Swal.fire({
        title: "Traitement du retrait...",
        html: `
    <div style="padding-top:10px;">
      <div class="swal2-loader" style="
  ...
  `,
      ...
      await fetch("https://amwalletapi.onrender.com/api/quiz/send-mail", {
        method: "POST",
        headers: {
